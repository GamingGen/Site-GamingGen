...
AppControllers.controller('AdminListNonBanUsersCtrl', ['$scope', '$http', 'socket', 'usersService', function($scope, $http, socket, usersService) {
  // ----- Init -----
  var lstUsers  = this;
  ...
  $scope.banUser = function(user) {
    if (user !== undefined) {
      $http.post('/users/ban', {user : {id: user._id, pseudo: user.pseudo}}).then(function() {
