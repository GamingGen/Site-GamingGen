...
let articleEvent = function(ServerEvent) {
  ServerEvent.on('saveArticle', function(data, socket) {
    if (socket.request.session.passport.user && socket.request.session.passport.user.roles && socket.request.session.passport.user.roles.includes('Rédacteur')) {
  ...
  ServerEvent.on('rmArticle', function(data, socket) {
    if (socket.request.session.passport.user && socket.request.session.passport.user.roles && socket.request.session.passport.user.roles.includes('AdminRédacteur')) {
